...
import {
  FormDesignState,
  FormResponseDetailsState,
  stepState,
  submittedTestimonialState,
} from "@/app/atoms";
...
export default function RecordT() {
  ...
  const uploadVideoToSupabase = async (blob: Blob): Promise<string> => {
    ...
    const { data } = supabase.storage
      .from("videos")
      .getPublicUrl(uploadedVideo.path);
  ...
  return (
    <div
      className="w-full relative pt-12 pb-6 space-y-10 items-center smooth
        justify-center flex flex-col bg-white"
    >
      <img
        src={
          FormDesign.logo ? FormDesign.logo : "https://via.placeholder.com/60"
        }
        alt="logo"
        className="form_logo"
      />
      <div className="w-full space-y-3">
        ...
        <ReactMarkdown>{FormResponseDetails.videoPrompt}</ReactMarkdown>
      </div>
      <div
        className="w-full space-y-4 items-center justify-center flex 
      ...
      >
        {isRecording || recordedVideoUrl ? (
          <>
            ...
            />
            {isRecording ? (
              <button className="button_active" onClick={stopRecording}>
                Stop Recording
              </button>
            ) : null}
          </>
          ...
          <button
            className="p-6 rounded-full bg-red-500 smooth hover:scale-110 animate-pulse "
            onClick={startRecording}
          />
        )}
      </div>
      {publicVideoURL && (
        <button className="button_active" onClick={() => setStep(3)}>
          Next
        </button>
      )}
    </div>
